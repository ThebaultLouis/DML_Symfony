...
$d = new \Doctrine\ORM\Mapping\Driver\AnnotationDriver(($this->privates['annotations.cached_reader'] ?? $this->getAnnotations_CachedReaderService()), array(0 => ($this->targetDirs[3].'\\src\\Entity'), 1 => ($this->targetDirs[3].'\\vendor\\vich\\uploader-bundle\\Entity')));

$c->addDriver($d, 'App\\Entity');
$c->addDriver($d, 'Vich\\UploaderBundle\\Entity');

$a->setEntityNamespaces(array('App' => 'App\\Entity', 'VichUploaderBundle' => 'Vich\\UploaderBundle\\Entity'));
...
$a->setRepositoryFactory(new \Doctrine\Bundle\DoctrineBundle\Repository\ContainerRepositoryFactory(new \Symfony\Component\DependencyInjection\ServiceLocator(array('App\\Repository\\AdminRepository' => function () {
    return ($this->privates['App\Repository\AdminRepository'] ?? $this->load('getAdminRepositoryService.php'));
}, 'App\\Repository\\CoursSupplementaireRepository' => function () {
    return ($this->privates['App\Repository\CoursSupplementaireRepository'] ?? $this->load('getCoursSupplementaireRepositoryService.php'));
}, 'App\\Repository\\DanseAppriseRepository' => function () {
    return ($this->privates['App\Repository\DanseAppriseRepository'] ?? $this->load('getDanseAppriseRepositoryService.php'));
}, 'App\\Repository\\DanseReviseeRepository' => function () {
    return ($this->privates['App\Repository\DanseReviseeRepository'] ?? $this->load('getDanseReviseeRepositoryService.php'));
}, 'App\\Repository\\ManifestationRepository' => function () {
    return ($this->privates['App\Repository\ManifestationRepository'] ?? $this->load('getManifestationRepositoryService.php'));
}, 'App\\Repository\\NiveauRepository' => function () {
